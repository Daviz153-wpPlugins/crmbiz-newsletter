...
- [ ] **이력 페이지 상태/날짜 필터** — 완료/실패/대기/취소 상태별 필터 + 날짜 범위 선택. 현재 제목 검색만 가능. **[M]**
- [ ] **이력 서버사이드 정렬** — 현재 클라이언트 정렬은 현재 페이지 내 데이터만 정렬. API에 `sort_by` / `sort_dir` 파라미터 추가. **[M]**
- [ ] **대시보드 차트 기간 선택** — 현재 30일 고정. 7일 / 30일 / 90일 토글 추가. **[S]**
- [ ] **발송 실패 상세 원인 표시** — `failed` 상태 캠페인의 실패 이유를 이력 상세 패널에 표시. **[S]**
- [ ] **즉시 발송 버튼 결과 피드백** — 클릭 후 실제 발송 시작 여부 확인 (현재 성공 toast만). **[S]**
- [ ] **수신거부 이름 검색** — 현재 이메일 검색만. FluentCRM `fc_subscribers` JOIN해 이름 검색 추가. **[S]**
...
- [ ] **암호화 AES-GCM 업그레이드** — 현재 AES-256-CBC는 MAC 없어 위변조 탐지 불가. AES-256-GCM(인증 암호화)으로 전환. 기존 URL 하위 호환 처리 포함. **[M]**
